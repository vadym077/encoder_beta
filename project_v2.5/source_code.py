...
# Функція для вставки тексту з буферу обміну в поле вводу
def paste_from_clipboard():
    text_from_clipboard = pyperclip.paste()
    input_field.delete(0, tk.END)
    input_field.insert(0, text_from_clipboard)
...
input_field = ttk.Entry(frame, state="normal")
...
copy_button = ttk.Button(frame, text="Копіювати 📄", command=lambda: pyperclip.copy(output_field.get()))
copy_button.grid(row=2, column=2)

clear_button = ttk.Button(frame, text="Очистити ❌", command=clear_fields)
clear_button.grid(row=1, column=2)

paste_button = ttk.Button(frame, text="Вставити  ✅", command=paste_from_clipboard)
paste_button.grid(row=3, column=2)
...
paste_button.config(style="Rounded.TButton")
